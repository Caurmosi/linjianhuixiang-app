...
def confidence_of(activity: float, mean_conf: float, species_count: int, duration_sec: float | None = None) -> float:
    ...
    - species_count   ：证据充分度，min(1, species_count/3)，3 种封顶；
    - duration_sec    ：采样充分度，min(1, duration/30)，30s 封顶（None 按 0）。

    confidence = 0.35*activity + 0.30*mean_conf + 0.20*min(1,species/3) + 0.15*min(1,dur/30)
    """
    ...
    species_term = min(1.0, max(0, int(species_count)) / 3.0)
    dur_term = min(1.0, max(0.0, float(dur)) / 30.0)
    raw = 0.35 * act + 0.30 * conf + 0.20 * species_term + 0.15 * dur_term
    return round(max(0.0, min(1.0, raw)), 2)


def confidence_label_of(confidence: float) -> str:
    """置信度等级：≥0.6 高 / ≥0.4 中 / <0.4 低（与前端 confidenceLabelOf 阈值一致）。"""
    if confidence >= 0.6:
        return "高"
    if confidence >= 0.4:
        return "中"
